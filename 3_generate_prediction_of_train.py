...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate predictions for the training/example dataset with the saved models."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_PREPARED_DATASET,
        help="Dataset to score. Defaults to the prepared example dataset.",
    )
    parser.add_argument(
        "--results-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional training metadata path.",
    )
    parser.add_argument(
        "--output-name",
        default="train_full_predictions.csv",
